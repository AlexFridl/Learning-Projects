...
<div class="container">
    ...
    <reset-form :old="{{json_encode(Session::getOldInput())}}" :email="{{json_encode(["email" => $email ?? null])}}" inline-template>
        <form @submit="validate" action="{{ route('password.update') }}" method="POST" class="mt-5">
            @csrf
            <input type="hidden" name="token" value="{{ $token }}">
            <div class="row">
                <div class="form-group col-lg-6 offset-lg-3">
                    <label for="email">Email</label>
                    <input v-model="inputs.email" id="email" type="email" class="form-control @error('email') is-invalid @enderror" :class="isInvalid('email')" name="email" value="{{ $email ?? old('email') }}" required autocomplete="email" autofocus>
            ...
            <div class="row mt-3">
                <div class="form-group col-lg-6 offset-lg-3">
                    <button class="btn btn-warning w-100" :disabled="isDisabled">Resetuj lozinku</button>
                </div>
            </div>
            <div class="row mt-1">
                <div class="form-group col-lg-6 offset-lg-3">
